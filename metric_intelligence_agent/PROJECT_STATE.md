...
- **Hosted test deployment support.** `APP_DEPLOYMENT_MODE=hosted` (or the same Streamlit secret) restricts the UI to the bundled synthetic TPC-H demo, hides custom-source setup, requires a password-masked per-session tester API key before query execution, and presents friendly authentication and unexpected-error messages. Repository-level Streamlit configuration suppresses browser-visible exception details. Connecting `main` to Streamlit Community Cloud and adding the live URL remain owner actions.
