...
3. The user receives the verified result with a plain-English explanation plus the data layer and cost metadata. The explanation may describe the applicable definition and assumptions, but that disclosure is currently LLM-generated and is not guaranteed. Unsupported or unresolved questions produce a clear limitation or failure instead of an unverified answer.
4. The user can rate the answer so its quality can be monitored.

For custom sources, the current product can connect supported databases, discover schema, collect and persist analyst layer classifications, generate and review a catalog, and import structured metric definitions for review and saving. Pipeline/dashboard SQL import, manual metric entry, setup validation, explicit activation, and previous-session reload remain deferred. The intended activation behavior is already settled: once those capabilities are completed and the required context exists, an analyst explicitly activates the source; it becomes query-active immediately, question-specific conversation, cache, feedback, and prior-result state are cleared on either source switch, and evaluation history is retained.
...
- **Explainable answers.** Users currently see the data layer and cost metadata, with an LLM-generated explanation that may include the definition and assumptions. Guaranteed structured disclosure remains the intended future behavior.
...
- Surface the queried data layer and cost metadata. Explanation text may include the definition used and assumptions made, but this LLM-generated disclosure is not currently guaranteed.
...
- Validate both referenced tables and referenced columns during structured metric import, and prevent ungrounded warnings from being bypassed when saving.
- Complete the custom-source workflow for pipeline/dashboard SQL import, manual metric entry, setup validation, explicit activation, and previous-session reload. This extends the existing connection, schema discovery, catalog, classification, and structured-import capabilities.
