...
For custom sources, the current product can connect supported databases, discover schema, collect and persist analyst layer classifications, generate and review a catalog, import layered metric definitions from pipeline/dashboard SQL, dbt manifests, and other structured formats, and let analysts review parsed facts, annotate or delete metrics, and correct relationships. Analysts can persist and run known-answer validation questions, record final human pass/fail decisions, and explicitly activate a passing custom source. Activation makes it query-active immediately; question-specific conversation, cache, feedback, and prior-result state are cleared on either source switch, while evaluation history is retained. Previous-session custom-connection reload remains deferred. Blank-form manual authoring of raw metric facts remains intentionally outside the product.
